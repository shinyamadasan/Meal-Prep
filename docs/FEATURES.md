...
- **Meal-prep flow card** — Implemented (D-076, extended D-078) · `renderMealPrepFlowCard()` ·
  ...
  signal.
- **What needs attention?** — Implemented (D-078, compact-always D-079) · `#dash-attention`, a
  `<details>` — compact ALWAYS by default (one summary line of counts, e.g. "2 expired · 1 use
  soon"), including when an expired item exists. Opens only via an explicit `.dash-attn-review-btn`
  ("Review", force-opens rather than toggling) or `openAttentionView()` (the notification/deep-link
  path, also reached from the global freshness banner's own View action through
  `viewFreshnessDetails()`, so the banner never duplicates itself with a second always-open
  summary). Manual open/close state otherwise survives re-renders. Content is unchanged: **Expired**
  (pantry + cooked food, each with one-tap `Keep` / `Remove`, plus a bulk **Remove expired (N)**),
  **Use soon** (≤2d, informational only — never bulk-removable), low-staple alerts, and **"Use
  soon"** recipe suggestions. Sourced from `collectAttentionItems()`. See DECISIONS D-057, D-079.
- **Record leftovers/takeout** — Implemented (D-079) · one compact row (`.dash-leftover-compact`,
  "+ Record leftovers / takeout") instead of a full card; same `openManualCookedModal()`.
...
- **Freshness alert banner** — Working · top-of-app on load; dismissable per session. Items marked `Keep` today are excluded from the expired count (`getFreshnessAlerts()`). Its **View** action opens Home's own attention detail (`viewFreshnessDetails()` → `openAttentionView()`), not the raw Fridge tab — see DECISIONS D-079.
...
- **Prepared Flavors in My Fridge (D-075)** — Implemented · branch `wave/fridge-prepared-flavors-and-inventory-check`, held for review, NOT merged · `renderFridgePreparedFlavors()` mirrors the Flavor Library's prepared-stock cards (same `AppState.preparedFlavors`, same `preparedFlavorCardHtml()`, same `useOnePreparedFlavor()` Used 1) into a "Prepared Flavors" block on My Fridge, so seeing physical sauce/marinade stock no longer requires a tab switch. Sort: fridge before freezer, then earlier truthful `expiresAt` first, then name. Omitted entirely (no empty card) when there is no prepared stock. Zero new state.
- **Last full inventory check (D-075)** — Implemented · branch `wave/fridge-prepared-flavors-and-inventory-check`, held for review, NOT merged · `verifyInventoryChecked()` / `renderInventoryVerifiedStatus()` · a one-tap manual confidence stamp ("Inventory checked N days ago" / "Inventory not yet verified") answering "when did I last physically reconcile the fridge/freezer/pantry?" for a multi-person household. `AppState.inventoryVerifiedAt` is a single scalar timestamp — no schedule, no reminder, no confidence scoring, no household accounts.
